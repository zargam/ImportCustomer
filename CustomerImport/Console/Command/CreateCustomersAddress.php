...
use Zagam\CustomerImport\Model\Customer\ProfileAdressFactory;
use Magento\Framework\Exception\LocalizedException;

class CreateCustomersAddress extends Command
{
    protected $importer;

    protected $profileAdressFactory;
     ...
     * @param ProfileAdressFactory $profileAdressFactory
     * @param StoreManagerInterface $storeManager
     */
    public function __construct(
        ProfileAdressFactory $profileAdressFactory,
        \Zagam\CustomerImport\Model\Address $address,
        ...
        $this->profileAdressFactory = $profileAdressFactory;
    ...
    */
    protected function configure(): void
    {
        ...
        $this->setDescription("Importing Customers Address via CSV & JSON");
        $this->setHelp('Run this command to importing Customers Address via CSV & JSON.');
        $this->setDefinition([
            new InputArgument(ImportInterface::PROFILE_NAME, InputArgument::REQUIRED, "Profile Name: sample-address-csv"),
            new InputArgument(ImportInterface::SOURCE, InputArgument::REQUIRED, "Source: sample_address.csv")
        ]);
        parent::configure();
    }
           ...
           if($file_extension !== end($file_path_end_extension)){
            ...
            if ($importData = $this->getImporterInstance($profileType)->getImportData($input)) {
                ...
                foreach ($importData as $data) {
                    $this->address->createCustomerAddress($data, $websiteId, $storeId);
                    $progressBar->advance();
                }
                $progressBar->finish();
    ...
    protected function getImporterInstance($profileType): ImportInterface
    {
        if (!($this->importer instanceof ImportInterface)) {
            $this->importer = $this->profileAdressFactory->create($profileType);
        }
